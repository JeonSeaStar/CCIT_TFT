...
public class BackendRank
{
    ...
    {
        //'복사한 UUID 값' 을 '뒤끝 콘솔 > 랭킹 관리'에서 생성한 랭킹의 UUID값으로 변경해주세요.
        ...
        {
            Debug.LogError("데이터 조회 중 문제가 발생했습니다.");
            return;
        }
        ...
        Debug.Log("내 게임 정보의 rowInDate : " + rowInData);

        Param param = new Param();
        param.Add("level", score);

        //추출된 rowInDate를 가진 데이터에 param값을 수정을 진행하고 랭킹에 데이터를 업데이트 합니다.
