...
public class PoisonBomb : Piece
{
    ...
    public override void DoAttack()
    {
        ...
        {
            invincible = false;
            SoundManager.instance.Play("Bomb_Series/S_Attack_PoisonBomb", SoundManager.Sound.Effect);
            Damage(attackDamage);
            mana += manaRecovery;
            StartNextBehavior();
        }
    ...
    public void BombSkill(float damage, float tickDamage, float time)
    {
        if (dead)
            return;
        SkillState();
        SoundManager.instance.Play("Bomb_Series/S_Skill_PoisonBomb", SoundManager.Sound.Effect);
    ...
    {
        SoundManager.instance.Play("Bomb_Series/S_Death_PoisonBomb", SoundManager.Sound.Effect);
        StopAllCoroutines();
