...
{
    public GameObject[] userObject = new GameObject[8];
    ...
    void Start()
    {
        ...
        // 오브젝트 끄는 건 스위치 문으로 해야 할듯
        if (numOfClient != 4 && matchInstance.nowModeType != MatchModeType.Melee)
        {
        ...
        foreach (var record in matchInstance.gameRecords.OrderByDescending(x => x.Key))
        {
            ...
            var data = userObject[index++].GetComponentsInChildren<TMP_Text>();
            data[0].text = name;
            data[1].text = score;
