...
{
    private readonly int increaseExperiece = 25; //게임 플레이 당 획득 경험치
    ...
    public void Process()
    {
        ...
        BackendGameData.Instance.UserGameData.experience += increaseExperiece;

        //현재 경험치가 최대 경험치보다 크거나 같고, 현재 레벨이 최대 레벨보다 작을 때
        if (BackendGameData.Instance.UserGameData.experience >= BackendChartData.levelChart[currentLevel - 1].maxExperience && BackendChartData.levelChart.Count > currentLevel)
        {
            //레벨업 보상 지급
